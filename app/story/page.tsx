    ...
    <div className={`min-h-screen transition-colors duration-300 ${isDark ? 'bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950' : 'bg-white'}`}>
      ...
      <div className="pt-20 min-h-screen">
        <div className="flex flex-col md:flex-row h-[calc(100vh-5rem)]">
          {/* Left Side - Scrollable Story Text */}
          <motion.div
          ...
          >
            <div className={`max-w-7xl mx-auto px-6 sm:px-8 lg:px-12 py-6 lg:py-12 space-y-8 text-lg sm:text-xl leading-relaxed transition-colors duration-300 ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>
              <div className="mb-8">
          ...
          {/* Right Side - Fixed Card with Image and Profile Info */}
          <motion.div
            initial={{ opacity: 0, x: 20 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ duration: 0.6, delay: 0.3 }}
            className={`md:sticky md:top-20 md:h-[calc(100vh-5rem)] md:overflow-y-auto w-full md:w-1/4 lg:w-1/3 px-6 sm:px-8 lg:px-12 py-6 lg:py-8 transition-colors duration-300 ${isDark ? 'bg-slate-900/50' : 'bg-slate-50/50'}`}
          >
            <div className="flex flex-col items-center justify-center h-full space-y-4">
              {/* Profile Image - Smaller */}
              <div className="relative w-full max-w-xs aspect-square rounded-2xl overflow-hidden border-4 shadow-xl transition-all duration-300 border-primary-500/50 hover:border-primary-500 hover:shadow-primary-500/20">
                <Image
              ...
              <div className="text-center">
                <h2 className={`text-2xl sm:text-3xl font-medium mb-2 transition-colors duration-300 ${isDark ? 'text-white' : 'text-slate-900'}`}>Fathima Zahra</h2>
